function (file_name, qaqc_location, extra_historical_days) 
{
  ...
  obs_list <- list()
  full_time_local_extended <- seq(full_time_local[1] - lubridate::days(extra_historical_days), 
                                  max(full_time_local), by = "1 day")
  for (i in 1:length(obs_config$state_names_obs)) {
    message("Extracting ", obs_config$target_variable[i])
    obs_tmp <- array(NA, dim = c(length(full_time_local_extended), 
                                 length(depths)))
    for (k in 1:length(full_time_local_extended)) {
      for (j in 1:length(depths)) {
        ...
        d1 <- d1 %>% dplyr::filter(date == lubridate::as_date(full_time_local_extended[k]))
        if (nrow(d1) == 0) {
          warning("No observations for ", obs_config$target_variable[i], 
                  " on ", lubridate::as_date(full_time_local_extended[k]))
        }
        d1 <- d1 %>% dplyr::filter((is.na(hour) | hour == 
                                      lubridate::hour(full_time_local_extended[k])))
        if (nrow(d1) == 0) {
          warning("No observations for ", obs_config$target_variable[i], 
                  " on ", lubridate::as_date(full_time_local_extended[k]), 
                  " at ", lubridate::hour(full_time_local_extended[k]), 
                  ":00:00")
        }
        d1 <- d1 %>% dplyr::filter(abs(depth - depths[j]) < 
  ...
  obs <- array(NA, dim = c(length(full_time_local_extended), 
                           length(depths), length(obs_names)))
  for (i in 1:length(obs_names)) {
    obs[, , i] <- obs_list[[i]]
  }
  return(list(obs = obs, full_time_local_extended = full_time_local_extended, 
              ...
              full_time_local = full_time_local, obs_long = d, depths = depths, 
              obs_names = obs_names))
}
